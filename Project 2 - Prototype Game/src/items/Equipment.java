...
public class Equipment extends Item {
	...
	public void addAttributeBonus(AttributeBonus b) {
		...
		//this snippet removes all the null spaces
		for (int j = 0; j < this.all_bonuses.length; j++) {
			if (all_bonuses[j] == null) {
				AttributeBonus[] new_bonus_arr = new AttributeBonus[all_bonuses.length-1];
				for (int k = 0; k < this.all_bonuses.length; k++) {
					if (k < j) {
						new_bonus_arr[k] = this.all_bonuses[k];
					} else if (k > j) {
						new_bonus_arr[k] = this.all_bonuses[k-1];
					}
				}
				this.all_bonuses = new_bonus_arr;
			}
